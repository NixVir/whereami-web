...
function initializeDateSelectors() {
    ...
    // Populate years (1900 to current year + 100)
    const currentYear = new Date().getFullYear();
    for (let year = currentYear + 100; year >= 1900; year--) {
...
function handlePresetClick(event) {
    ...
    const presetDate = button.getAttribute('data-date');
    const eventName = button.getAttribute('data-name');

    console.log('Preset clicked:', eventName, presetDate);

    const birthDateInput = document.getElementById('birth-date');

    // For extreme future dates beyond year 9999, handle specially
    if (presetDate.length > 10) {
        // Store the extreme date in a special way
        birthDateInput.setAttribute('data-extreme-date', presetDate);
        birthDateInput.setAttribute('data-event-name', eventName);
        // Set a placeholder date
        birthDateInput.value = '9999-12-31';

        // Update visible selects
        document.getElementById('birth-year').value = '9999';
        document.getElementById('birth-month').value = '12';
        document.getElementById('birth-day').value = '31';
    } else {
        // Regular date - parse and set in both hidden input and selects
        const [year, month, day] = presetDate.split('-');

        birthDateInput.value = presetDate;
        birthDateInput.removeAttribute('data-extreme-date');
        birthDateInput.removeAttribute('data-event-name');
...
async function handleCalculate() {
    ...
    try {
        ...
        currentData = result;
        currentData.event_name = eventName; // Store event name for display
...
function showResults(data) {
    ...
    try {
        ...
        let summaryText;
        if (data.event_name) {
            // Show event name for preset events
            summaryText = `${data.event_name}: ${formatTimeSpan(years)} years`;
        } else {
            // Show age only
            summaryText = `Age: ${formatTimeSpan(years)} years`;
        }

        document.getElementById('journey-summary').textContent = summaryText;
...
function handleTimelineChange(event) {
    ...
    // Set journey progress to slider position
    visualization.setJourneyProgress(progress);
}

function formatTimeSpan(years) {
    if (years >= 1e9) {
        return (years / 1e9).toFixed(2) + ' billion';
    } else if (years >= 1e6) {
        return (years / 1e6).toFixed(2) + ' million';
    } else if (years >= 1e3) {
        return (years / 1e3).toFixed(2) + ' thousand';
    }
